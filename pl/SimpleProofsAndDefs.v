...
Lemma sub_cube: forall n m: Z,
  n ^3- m ^3 = (n - m) * (n ^2 + n * m + m ^2).
Proof.
  intros.
  lia.
Qed.
Lemma mul_ne_pos: forall n m: Z,
  n <= 0 -> m >= 0 -> n * m <= 0.
Proof.
  intros.
  lia.
Qed.
Lemma sum_of_sqr_lt1: forall n m: Z,
  n <= m -> n ^2 +n*m+ m ^2 >= 0.
Proof.
  intros.
  pose proof sqr_pos (n+m) as Hnm.
  lia.  
Qed.
Lemma sub_cube_le: forall n m: Z,
  n <= m -> n ^3 <= m ^3.
Proof.
  intros.
  pose proof sub_cube n m as Hnm.
  pose proof sum_of_sqr_lt1 n m as Hnm1.
  pose proof mul_ne_pos (n-m) (n*n+n*m+m*m) as Hnm2.
  lia.
Qed.
...
Proof.
  unfold mono.
  intros n m H.
  pose proof sub_cube_le n m as Hnm.
  lia.
Qed.
